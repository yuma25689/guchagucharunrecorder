...
import app.guchagucharr.guchagucharunrecorder.ControlDefs;
import app.guchagucharr.guchagucharunrecorder.DisplayBlock;
...
import app.guchagucharr.guchagucharunrecorder.PagerHandler;
...
import app.guchagucharr.guchagucharunrecorder.edit.EditActivity;
...
{
	// =============== 定数
	static final int CONTEXT_MENU_EDIT_ID = 1;
	
	// 他のActivityから渡されてくる値のキー
	...
	private static final String TAG = "ResultAcitivity";
	...
	void resetComponentContainer()
	{
        ...
        // FIXME: onResumeの時に呼ばれないとやばい
 	   	ViewTreeObserver viewTreeObserver = componentContainer.getViewTreeObserver();
	    viewTreeObserver.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
	        @Override
	        public void onGlobalLayout() {
	        	LogWrapper.w(TAG,"onGlobalLayout");
	        	sendInitSizeHasGottenMessage();
	        }
	    });        
		
	}

	void sendInitSizeHasGottenMessage()
	{
        widthTmp = componentContainer.getWidth();
        heightTmp = componentContainer.getHeight();
    	LogWrapper.v("sendInitSizeHasGottenMessage","width = "+ widthTmp + "height = " + heightTmp);
...
//	}		
	...
	/**
	 * ページャの初期化
	 * onResume後に、Windowのサイズ情報が取得できたらハンドラから呼ばれるはず
	 * @return
	 */
	@Override
	public int initPager()
	{
		...
		// ViewPagerを設定する
        mViewPager = (ViewPager)this.findViewById(R.id.viewpager1);
        adapter = new ResultPagerAdapter(this, this);
        mViewPager.setAdapter(adapter);
        return 0;
	}
	/**
	 * PagerAdapterから呼ばれるらしい
	 * ページ内の初期化
	 * @param position
	 * @param rl
	 * @return
	...
	public int initControls( int position, RelativeLayout rl )
	{
		...
		{
			LogWrapper.v(TAG,"initControls - main");
			// メインページ
			// FIXME: 最初はソースから作ればいいと思っていたが、
			// やっぱりXMLで作ればいいかもしれない。
...
//			addViewToCompContainer(activityTypeIcon);
			...
			if( false == m_bNoGpsMode ) {
				...
				if( RunLoggerService.getLogStocker().getTotalTime() != 0 )
		...
		{
			LogWrapper.v(TAG,"initControls - lap");
			...
			for( int i=0; i < RunLoggerService.getLogStocker().getStockedLapCount(); i++ )
			{
				...
				double speed = 0;
				long time = 0;
				distance = lapData.getAppropriteDistance();
				time = lapData.getAppropriteTime();
				if( time != 0 )
					speed = distance / ( time * UnitConversions.MS_TO_S ); 
				
				LogWrapper.v(TAG,"initControls - lap distance=" + distance + " time=" + time);
				// DisplayBlock追加
				if( 1 < RunLoggerService.getLogStocker().getStockedLapCount() )
				{
				...
				DisplayBlock dispBlock = new DisplayBlock(
						this,
						dispInfo.getXNotConsiderDensity(
								componentContainer.getWidth()),
						dispInfo.getYNotConsiderDensity(
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        switch (requestCode) {
            case REQ_CODE_EDIT:
            	LogWrapper.v(TAG, "onActivityResult - REQ_CODE_EDIT");
                if (resultCode == EditActivity.EDIT_DONE) 
                {
                	LogWrapper.v(TAG, "EDIT_DONE");
                    handler.clearFlags();
                    resetComponentContainer();
                    // きっと、この後onResumeが起きて下記が行われると思っている・・・。
                    sendInitSizeHasGottenMessage();
//                    dispInfo.init(this, componentContainer, handler, true);
                }
                break;
